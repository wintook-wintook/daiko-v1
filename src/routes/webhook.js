...
const {procesarMensajeTelegram, configurarWebhookTelegram, obtenerInfoBot, enviarMensajeTelegram } = require('../tools/telegram');
...
console.log({ln: 22});
  ...
  app.get('/telegram/status', (req, res) => {
    res.json({
      ...
      bot_token_configured: !!TELEGRAM_BOT_TOKEN && TELEGRAM_BOT_TOKEN !== 'TU_BOT_TOKEN_AQUI',
      endpoints: {
...
//console.log({token: req.query.token, account_id: req.body.account.id, conversation_id: req.body.conversation.id});      
      ...
      // Procesar el mensaje
      Object.assign(webhookData, {token: req.query.token, account_id: req.body.account.id, conversation_id: req.body.conversation.id});
      const result = await procesarMensajeWebhook(webhookData);
      
      Object.assign(result, {token: req.query.token, account_id: req.body.account.id, conversation_id: req.body.conversation.id});
// console.log({result});
      if (!result.success) {
